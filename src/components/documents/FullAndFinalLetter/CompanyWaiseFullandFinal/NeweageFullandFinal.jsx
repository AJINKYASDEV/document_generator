...
const NeweageFullAndFinal = ({ company, data }) => {
  ...
  const basic = +(gross * 0.408).toFixed(2);
  const hra = +(gross * 0.18).toFixed(2);
  const da = +(gross * 0.16).toFixed(2);
  const special = +(gross * 0.12).toFixed(2);
  const facility = +(gross * 0.08).toFixed(2);
  const food = +(gross * 0.052).toFixed(2);

  const earned = (v) => +(v * ratio).toFixed(2);

  const totalActual = basic + hra + da + special + facility + food;
  const totalEarned =
    earned(basic) +
    earned(hra) +
    earned(da) +
